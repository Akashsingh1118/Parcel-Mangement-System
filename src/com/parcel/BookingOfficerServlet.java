...
@WebServlet("/BookingOfficerServlet")
public class BookingOfficerServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        
    	HttpSession session = request.getSession(false);
        String userId = (String) session.getAttribute("officerUsername");
        if (userId == null) {
            response.sendRedirect("officer-login.jsp"); // Redirect to login if session is invalid
            return;
        }
        ...
        try {
            ...
            request.getRequestDispatcher("booking-officer.jsp").forward(request, response);
